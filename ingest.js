...
async function ingestFile(db, logFile, force) {
  ...
  const existing = db.prepare('SELECT * FROM ingestion_log WHERE log_file = ?').get(logFile);

  if (existing && !force) {
    if (existing.file_size === stat.size) {
      console.log(`[skip] ${logFile} — already ingested (${Number(existing.row_count).toLocaleString()} rows)`);
      return 0;
    }
    console.log(`[update] ${logFile} — file size changed, re-ingesting...`);
    db.exec(`DELETE FROM logs WHERE log_file = '${logFile.replace(/'/g, "''")}'`);
  ...
  console.log(`[ingest] ${logFile} (${(stat.size / 1024 / 1024).toFixed(0)} MB)...`);
    ...
    if (m) {
      ...
      if (pending) {
        ...
        if (batch.length >= BATCH_SIZE) {
          ...
          process.stdout.write(`\r  ${total.toLocaleString()} rows | ${elapsed}s elapsed`);
        }
      }
  ...
  db.prepare('INSERT OR REPLACE INTO ingestion_log VALUES (?, ?, ?, ?)').run(
    logFile, new Date().toISOString(), total, stat.size
  );
...
async function main() {
  ...
  if (checkOnly) {
    const rows = db.prepare('SELECT * FROM ingestion_log ORDER BY log_file').all();
    if (!rows.length) { console.log('No files ingested yet.'); }
    else {
      console.log('Ingested files:');
      for (const r of rows)
        console.log(`  ${r.log_file}: ${Number(r.row_count).toLocaleString()} rows, ingested ${r.ingested_at}`);
    }
    db.close();
    return;
  }
  ...
  if (!files.length) { console.log('No log files found in ./logs/'); db.close(); return; }
