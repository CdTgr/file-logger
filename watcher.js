...
function buildRow(logFile, parsed, msgRaw) {
  const row = {
    log_file: logFile, timestamp: parsed.iso, timestamp_unix: parsed.unix,
    level: 'INFO', level_num: null, message: msgRaw,
    method: null, url: null, status_code: null,
    response_time: null, pid: null, hostname: null, req_id: null,
  };
  if (msgRaw.startsWith('{')) {
    try {
      ...
      if (j.req) { row.method = j.req.method ?? null; row.url = j.req.url ?? null; }
      if (j.res)   row.status_code   = j.res.statusCode ?? null;
      row.response_time = j.responseTime ?? null;
    } catch { row.level = detectLevel(msgRaw, null); }
  } else {
...
class LogWatcher {
  ...
  start() {
    ...
    const ingested = this.db.prepare('SELECT log_file, file_size FROM ingestion_log').all();
    for (const r of ingested) {
      this.state.set(r.log_file, { offset: Number(r.file_size), pending: null });
    }

    // Catch up on any growth since last ingest/watch
    const existing = fs.readdirSync(LOGS_DIR).filter(f => f.endsWith('.log') || f.endsWith('.txt'));
    for (const f of existing) this._catchUp(f);

    // Watch directory for new files appearing
    this._dirWatcher = fs.watch(LOGS_DIR, (event, filename) => {
      if (filename && (filename.endsWith('.log') || filename.endsWith('.txt'))) {
        if (!this._fileWatchers.has(filename)) this._watchFile(filename);
        this._scheduleRead(filename);
      }
    });

    // Watch each existing file
    for (const f of existing) this._watchFile(f);
    ...
    if (POLL_MS > 0) {
      ...
      this._pollTimer.unref(); // don't keep process alive just for polling
    }

    const count = existing.filter(f => fs.statSync(path.join(LOGS_DIR, f)).size > 0).length;
    const pollNote = POLL_MS > 0 ? ` + polling every ${POLL_MS / 1000}s` : '';
    console.log(`[watcher] Watching ${count} log file(s) for changes (fs.watch${pollNote})`);
  }

  stop() {
    clearInterval(this._pollTimer);
    this._dirWatcher?.close();
    for (const w of this._fileWatchers.values()) w.close();
    this.db.close();
  }

  _poll() {
    let files;
    try { files = fs.readdirSync(LOGS_DIR).filter(f => f.endsWith('.log') || f.endsWith('.txt')); }
    catch { return; }

    for (const f of files) {
      // Register watcher for files that appeared since last poll
      if (!this._fileWatchers.has(f)) this._watchFile(f);
      this._catchUp(f);
    }
  }
  ...
  _watchFile(logFile) {
    ...
    const w = fs.watch(filePath, (event) => {
      if (event === 'change') this._scheduleRead(logFile);
    });
    w.on('error', () => {}); // ignore errors from deleted files
    this._fileWatchers.set(logFile, w);
  }
  ...
  _readNew(logFile) {
    ...
    if (committed.length) {
      ...
      console.log(`[watcher] ${logFile} +${committed.length} new entr${committed.length === 1 ? 'y' : 'ies'}`);
    }
  ...
  _insertRows(logFile, rows, newFileSize) {
    ...
    try {
      ...
      ).all(logFile, rows.length).map(r => Number(r.id));
      if (ids.length) {
        this.db.exec('BEGIN');
        const ftsInsert = this.db.prepare('INSERT INTO logs_fts(rowid, message) VALUES (?, ?)');
        for (const id of ids) {
          const msg = this.db.prepare('SELECT message FROM logs WHERE id = ?').get(id)?.message ?? '';
    ...
    const exists = this.db.prepare('SELECT 1 FROM ingestion_log WHERE log_file = ?').get(logFile);
    if (exists) {
      this.db.prepare('UPDATE ingestion_log SET file_size = ?, row_count = row_count + ?, ingested_at = ? WHERE log_file = ?')
        .run(newFileSize, rows.length, new Date().toISOString(), logFile);
    } else {
      this.db.prepare('INSERT INTO ingestion_log VALUES (?, ?, ?, ?)').run(logFile, new Date().toISOString(), rows.length, newFileSize);
    }
  }
}
